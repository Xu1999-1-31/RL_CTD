...
class MOSAC(MOPolicy, MOAgent):
    ...
    def __init__(
        ...
        if self.autotune:
            ...
            # self.target_entropy = -th.prod(th.Tensor(env.action_space.shape).to(self.device)).item() * 0.5
            ...
            self.log_alpha = th.tensor([np.log(initial_alpha)], dtype=th.float32, requires_grad=True, device=self.device)
    ...
    def train(self, total_timesteps: int, eval_env: Optional[gym.Env] = None, start_time=None, reward_log=True):
        ...
        for step in range(total_timesteps):
            ...
            obs = self.env.unwrapped.get_current_obs()
            ...
            next_obs, rewards, terminated, tns = self.env.step(actions)
            log_str = f"_{self.id}" if self.id is not None else ""
            to_log = {f"TNS{log_str}/tns": tns}
            wandb.log(to_log)
            ...
            self.buffer.add(obs=obs, next_obs=next_obs, action=actions, reward=rewards, done=terminated)
            #print(rewards)
            if reward_log == True:
                ...
                log_str = f"_{self.id}" if self.id is not None else ""
                to_log = {f"Delta_TNS{log_str}/Reward[0]": rewards_to_log[0]}
                wandb.log(to_log)
