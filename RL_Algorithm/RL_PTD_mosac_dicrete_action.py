...
class MOSoftQNetwork(nn.Module):
    ...
    def __init__(
        ...
        self.critic = mlp(
            input_dim=128 + 1, # obs output + action
...
class RL_PTD(MOPolicy, MOAgent):
    ...
    @override
    def update(self):
        for n in range(self.update_frequency):
            (mb_obs, mb_act, mb_rewards, mb_next_obs, mb_dones) = self.buffer.sample(
                self.batch_size, device=self.device
            )
            with th.no_grad():
                next_state_actions, log_probs, action_probs = self.actor.get_action(mb_next_obs)
                # (!) Q values are scalarized before being compared (min of ensemble networks)

                qf1_next_target = (self.qf1_target(mb_next_obs, next_state_actions) * self.weights_tensor).sum(dim = -1)
                qf2_next_target = (self.qf2_target(mb_next_obs, next_state_actions) * self.weights_tensor).sum(dim = -1)
            ...
            qf1_a_values = (self.qf1(mb_obs, next_state_actions) * self.weights_tensor).sum(dim = -1).gather(1, mb_act.long()).squeeze(-1)
            qf2_a_values = (self.qf2(mb_obs, next_state_actions) * self.weights_tensor).sum(dim = -1).gather(1, mb_act.long()).squeeze(-1)
            ...
            if n % self.policy_freq == 0:  # TD 3 Delayed update support
                for _ in range(self.policy_freq):  # compensate for the delay by doing 'actor_update_interval' instead of 1
                    ...
                    # (!) Q values are scalarized before being compared (min of ensemble networks)
                    qf1_values = (self.qf1(mb_obs, next_state_actions) * self.weights_tensor).sum(dim = -1)
                    qf2_values = (self.qf2(mb_obs, next_state_actions) * self.weights_tensor).sum(dim = -1)
            ...
            if n % 1 == 0 and self.log:
                ...
                wandb.log(to_log, step=(self.global_step) * self.update_frequency + n)
        ...
        for step in range(total_timesteps):
            ...
            if self.global_step < self.learning_starts:
                ...
                for obs in obs_list:
                    ...
                    action, _, _ = self.actor.get_action(th_obs)
                    action = action[0].detach().cpu().numpy()
                    action_list.append(action)
                    th_obs_list.append(th_obs)

            
            next_obs_list, reward_list, done_list, state_vector = self.env.step(action_list)

            # TRY NOT TO MODIFY: save data to reply buffer; handle `final_observation`
            for obs, next_obs, action, reward, done in zip(obs_list, next_obs_list, action_list, reward_list, done_list):
                self.buffer.add(obs=obs, next_obs=next_obs, action=action, reward=reward, done=done)
            
            #print(rewards)
            
            print(f"Timing Step: {step}, TNS: {state_vector[1]}, WNS: {state_vector[3]}, DRC: {state_vector[5]}")
            
            log_str = f"_{self.id}" if self.id is not None else ""
            to_log = {f"TNS{log_str}/tns": state_vector[3], f"DRC{log_str}/drc": state_vector[5]}
            wandb.log(to_log, step=self.global_step * self.update_frequency)
            ...
            if reward_log == True:
                ...
                with open(csv_filename, mode='a', newline='') as file: 
                        ...
                        if not file_exists: 
                            writer.writerow(["action", "Reward"]) # Write header if file doesn't exist 
                        writer.writerow(["Step", self.global_step])
                ...
                with open(csv_filename, mode='a', newline='') as file: 
                        writer = csv.writer(file) 
                        writer.writerow(["TNS:", state_vector[1], "DRC:", state_vector[5]])
                
                rewards_to_log /= len(reward_list)
                log_str = f"_{self.id}" if self.id is not None else ""
            ...
            if self.global_step >= self.learning_starts:
                ...
                if self.log and self.global_step % 5 == 0:
                    ...
                    wandb.log(
                        {"charts/SPS": int(self.global_step / (time.time() - start_time)), "global_step": self.global_step * self.update_frequency}
                    )
