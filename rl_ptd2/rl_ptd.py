...
from RL_PTD_Space import GraphSpace
import time
...
class RL_PTD(gym.Env):
    ...
    def step(self, action_list):
        ...
        # get the TNS/DRC variation for each region
        ...
        self.Sub_H_congestion, self.Sub_V_congestion = DataBuilder.BuildCongestionData(self.current_design + '_eco', 128, self.row, self.column)
        # self.graph already update before, in self._Get_Region_TNS
        next_obs_list = self._get_obs_list()

        return next_obs_list, reward_list, [False] * (self.row * self.column -1) + [True], (wns, wns_eco, tns, tns_eco, drc, drc_eco)
...
if __name__ == "__main__": 
    ...
    for i in range(2):
        ...
        next_obs_list, reward_list, done, state_vector = env.step(action_list)
